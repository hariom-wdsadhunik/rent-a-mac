...
import { X, CheckCircle2, ShieldCheck, Sparkles, ExternalLink, Calendar } from 'lucide-react';
        ...
        <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          <div className={`p-4 rounded-xl border flex items-start gap-3 ${
            ...
            <div>
              <h4 className="text-sm font-semibold">
                {isAvailable ? 'Spot Available for Instant Reservation' : `Spot Occupied (${slot.status})`}
              </h4>
              <p className="text-xs text-gray-300 mt-0.5">
                {isAvailable
                  ? 'Reserve this advertising spot on the internet’s MacBook. All submissions are moderated within 24 hours.'
                  : slot.activeAd
          ...
          {slot.activeAd && (
            <div className="p-4 rounded-xl bg-gray-950 border border-gray-800 space-y-3">
              <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Active Creative</h4>
          ...
          {/* Slot Description */}
          <div>
            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Description & Placement</h4>
            <p className="text-sm text-gray-300">{slot.description}</p>
            <div className="mt-2 flex items-center gap-4 text-xs text-gray-400">
              <span>Display Resolution: <strong className="text-white">{slot.width}px × {slot.height}px</strong></span>
              <span>Grid Mapping: <strong className="text-white">{slot.gridArea}</strong></span>
            </div>
          </div>
          ...
          {isAvailable && (
            <div className="space-y-3 pt-2 border-t border-gray-800">
              <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Select Rental Duration</h4>
              <div className="grid grid-cols-3 gap-2.5">
                {[
                  ...
                  >
                    ...
                    <span className="block text-[10px] font-semibold text-blue-400 mt-1">{option.discount}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="p-4 rounded-xl bg-gray-950 border border-gray-800 space-y-2 text-xs">
                ...
                <div className="flex justify-between text-sm font-bold text-white pt-2 border-t border-gray-800">
                  ...
                  <span className="text-blue-400 text-base">${pricing.finalPrice.toFixed(2)} <span className="text-xs font-normal text-gray-400">(${pricing.pricePerDay.toFixed(2)}/day)</span></span>
                </div>
              </div>
            </div>
          )}
